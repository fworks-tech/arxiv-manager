...
def api_arxiv_search(
    ...
    try:
        ...
        return TEMPLATES.TemplateResponse(request, "_arxiv_search_results.html", {"papers": papers, "error": ""})
...
def api_arxiv_extract(
    ...
    try:
        ...
        return TEMPLATES.TemplateResponse(request, "_arxiv_figures.html", {"figures": figures, "error": ""})
    except Exception as e:
        logger.error("arxiv extract error: %s", e, exc_info=True)
        return TEMPLATES.TemplateResponse(request, "_arxiv_figures.html", {"figures": None, "error": str(e)[:200]})
